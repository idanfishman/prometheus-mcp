...
This example demonstrates a complete PostgreSQL monitoring setup using Prometheus,
Grafana, and a "bad" Go application that generates various database performance issues
...
The bad application is designed to create realistic database performance problems that
...
These are pre-configured in the docker-compose.yaml file but can be overridden if
needed.
...
2. **Wait for initialization** (about 30-60 seconds for the bad app to create tables and
   data)
...
This setup is perfect for testing the Prometheus MCP server's ability to diagnose
database issues. You can ask the MCP agent questions like:
...
The MCP server will query Prometheus metrics and provide intelligent analysis of the
database performance problems created by our bad application!
